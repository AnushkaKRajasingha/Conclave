        ...
        if ($q->num_rows() == 0) { // if no match account doest exist
            ...
            if ($row->timediff != 1) {
                ...
                if($this->db->affected_rows() == 1) { 
                	$pointdata = array(
	    			'user_id' =>  $row->user_id,
	    			'amount' => isset(Settings_model::$db_config['min_noof_point']) ? Settings_model::$db_config['min_noof_point'] : 0 ,
	    			'reason' => 'Accout activation',
	    			'earn_date' => date('Y-m-d H:i:s') ,
	    			'user_response' =>  $row->user_id
    	);
                	$this->db->insert(DB_PREFIX .'points', $pointdata);
